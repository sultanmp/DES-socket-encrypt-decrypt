...
def des_encrypt(plaintext, key):
    ...
    for round_key in round_keys:
        temp = right_half
        right_half = ''.join('1' if a != b else '0' for a, b in zip(left_half, f_function(right_half, round_key)))
        left_half = temp

    final_output = right_half + left_half
    ciphertext = permute(final_output, ip_inverse_table)
...
def main():
    ...
    try:
        ...
        # Combine plaintext and key into the format "key,plaintext"
        message = f"{key},{plaintext}"

        # Send combined message to the server
