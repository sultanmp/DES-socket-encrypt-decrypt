...
def main():
    ...
    while True:
        ...
        try:
            ...
            print("Data received (excluding key)")

            # Split the received data into key and plaintext (or ciphertext)
            key, text = data.split(',')
            key = '0E329232EA6D0D73'  

            # Check if text is binary (ciphertext) or plaintext
            if all(c in '01' for c in text):
                # Assume it's ciphertext, attempt decryption
                print(f"Ciphertext: {text}")
                plaintext_bin = des_decrypt(text, key)
                plaintext = bin_to_str(plaintext_bin)
                print(f"Decrypted plaintext: {plaintext}")
                conn.sendall(plaintext.encode('utf-8'))
            else:
                # Assume it's plaintext, attempt encryption
                print(f"Plaintext: {text}")
                plaintext_bin = str_to_bin(text)
                if len(plaintext_bin) % 64 != 0:
                    # Pad the plaintext to make it a multiple of 64 bits
